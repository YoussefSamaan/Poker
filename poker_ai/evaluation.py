...
def evaluate_five(cards: Iterable[Card | str]) -> HandRank:
    ...
    if len(hand) != 5:
        raise ValueError(f"five-card evaluation requires exactly 5 cards, got {len(hand)}")
...
def evaluate_holdem(cards: Iterable[Card | str]) -> HandRank:
    ...
    if not 5 <= len(available) <= 7:
        raise ValueError(f"Hold'em evaluation requires 5 to 7 cards, got {len(available)}")
